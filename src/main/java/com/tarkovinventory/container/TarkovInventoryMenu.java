...
public class TarkovInventoryMenu extends AbstractContainerMenu {
    ...
    public void broadcastChanges() {
        ...
        if (playerInventory.player instanceof ServerPlayer sp) {
            ...
            // Only rebuild the menu when slots need to APPEAR (equipping / resize).
            // When unequipping (→ 0), isActive()=false makes slots non-interactive
            // exactly like the rig system — no menu refresh needed, no cursor interference.
            if (sizeChanged && (rigColsNow > 0 || bpColsNow > 0)) scheduleMenuRefresh(sp);
