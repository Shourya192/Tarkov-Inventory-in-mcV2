...
public class TarkovInventoryMenu extends AbstractContainerMenu {
    ...
    private void createAllSlots() {
        ...
        // Inventory indices in Minecraft: 39=HEAD, 38=CHEST, 37=LEGS, 36=FEET
        int[] armorInvIdx = { 39, 38, 37, 36 };
        ...
        for (int i = 0; i < 4; i++) {
            addSlot(new TarkovSlot(playerInventory, armorInvIdx[i], armorX[i], armorY[i], armorTypes[i]));
        }

        // ── 3. Custom equipment slots (7 from capability) ──────
        // SLOT_PANTS (index 3) is hidden: vanilla LEGS armor already occupies
        // (EC3, 40). Showing both causes a double-slot overlap at the same pixel.
        ...
        // ── 4. Player main inventory (3×9 = 27) — off-screen ──────────
        // In Tarkov layout the player's stash is accessed through the
        // backpack grid and pockets, NOT a separate right panel.
        // Slots are registered so Forge can track items for shift-click,
        // but placed at -2000 so they are never rendered.
        ...
        addSlot(new Slot(playerInventory, 0, EC1, 140));   // PRIMARY
        addSlot(new Slot(playerInventory, 1, EC1, 160));   // SECONDARY
        for (int i = 0; i < POCKETS_COUNT; i++) {
            addSlot(new Slot(playerInventory, i + 2,
        ...
        for (int i = 0; i < rigSlotCount; i++) {
            ...
            addSlot(new TarkovSlot(rigContainer, i, sx, sy,
        ...
        for (int i = 0; i < backpackSlotCount; i++) {
            ...
            addSlot(new TarkovSlot(backpackContainer, i, sx, sy,
                    EquipmentSlotType.UNKNOWN) {
                @Override public boolean mayPlace(@NotNull ItemStack s) { return true; }
