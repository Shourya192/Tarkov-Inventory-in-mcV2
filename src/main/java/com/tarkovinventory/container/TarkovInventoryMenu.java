...
public class TarkovInventoryMenu extends AbstractContainerMenu {
    ...
    private void createAllSlots() {
        ...
        for (int i = 0; i < LOOT_SLOTS; i++) {
            ...
            addSlot(new Slot(lootContainer, i, sx, sy) {
                ...
                @Override public boolean mayPlace(@NotNull ItemStack s) {
                    if (fsx == -2000) return false;
                    // Corpse: allow placing into any active slot (rig/backpack content,
                    // gear, pockets, and flat container slots). Container loot slots too.
                    if (isCorpseSlot) return isActive();
                    return true;
                }
                @Override public boolean mayPickup(@NotNull Player p) {
    ...
    /**
     * Writes the rig/backpack grid contents into the gear ITEM's NBT (under
     * "TarkovRigInventory") so the items travel with the item when looted.
     * The display slots are MAX_COLS-indexed; we convert to the item's actual
     * cols×rows layout to match how RigContainer reads them back.
     */
    private void attachContentsToGearItem(TarkovCorpseBlockEntity be,
            Map<String, ItemStack> sl, String gearKey,
            List<ItemStack> maxColsItems, int cols, int rows) {
        ItemStack gearItem = sl.get(gearKey);
        if (gearItem == null || gearItem.isEmpty()) return;
        ...
        // Convert MAX_COLS display layout → tight cols×rows layout
        ...
        gearItem.getOrCreateTag().put("TarkovRigInventory", inv.serializeNBT());
        sl.put(gearKey, gearItem);
    ...
    private void writeLootBack(ServerPlayer sp) {
        ...
        if (activeLootIsCorpse) {
            ...
            // RIG CONTENTS [17..160] — stored MAX_COLS-indexed (same as display)
            ...
            // Re-attach contents to the rig ITEM's NBT so they travel when the item is taken.
            attachContentsToGearItem(be, sl, "cap.rig", rigItems, corpseRigCols, corpseRigRows);
            ...
            attachContentsToGearItem(be, sl, "cap.back", gridItems, corpseGridCols, corpseGridRows);
