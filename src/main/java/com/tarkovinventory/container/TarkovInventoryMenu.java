...
public class TarkovInventoryMenu extends AbstractContainerMenu {
    ...
    public  static final int LOOT_SLOTS    = 295;
    ...
    private void createAllSlots() {
        ...
        int pocketRowY    = LOOT_HEADER_H;                                            // 14
        int cRigY         = pocketRowY + CELL + 8 + LABEL_H;                         // 14+18+8+14=54
    ...
    public void broadcastChanges() {
        ...
        if (playerInventory.player instanceof ServerPlayer sp) {
            ...
            // Only rebuild the menu when EQUIPPING (at least one slot still has a live item).
            // When picking up / unequipping (→ 0), isActive()=false + grey placeholders
            // handle it instantly with no menu close/reopen, so the player keeps their cursor.
            if (sizeChanged && (rigColsNow > 0 || bpColsNow > 0)) {
    ...
    private void scheduleMenuRefresh(ServerPlayer sp) {
        ...
        sp.server.execute(() -> {
            ...
            // Return cursor item to pockets (hotbar 2-8), NOT vanilla Inventory.add()
            // which fills hotbar slot 0 (PRIMARY) or slot 1 (SECONDARY) first.
            ItemStack carried = sp.containerMenu.getCarried();
            if (!carried.isEmpty()) {
                boolean placed = false;
                for (int i = 2; i <= 8 && !placed; i++) {
                    if (sp.getInventory().getItem(i).isEmpty()) {
                        sp.getInventory().setItem(i, carried.copy());
                        placed = true;
                    }
                }
                if (!placed) sp.drop(carried.copy(), false);
