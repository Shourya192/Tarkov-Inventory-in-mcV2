...
public class TarkovInventoryMenu extends AbstractContainerMenu {
    ...
    public  static final int CORPSE_RIG_BASE    = 7;
    public  static final int CORPSE_GRID_BASE   = 151;
    public  static final int CORPSE_GEAR_BASE   = 295; // 8 equipped-item slots: rig,bp,head,chest,legs,feet,face,ear
    ...
    private TarkovInventoryMenu(int windowId, Inventory playerInv, ConstructorArgs a) {
        ...
        this.bpCols  = a.bpCols()  > 0 ? a.bpCols()  : backpackContainer.getCols();
        this.bpRows  = a.bpRows()  > 0 ? a.bpRows()  : backpackContainer.getRows();
    ...
    private static ConstructorArgs parseFromBuf(Player player, @Nullable FriendlyByteBuf data) {
        if (data == null) {
            ...
            return new ConstructorArgs(rc.getCols(), rc.getRows(), bp.getCols(), bp.getRows(),
    ...
    private static ConstructorArgs parseFromWorld(Player player, BlockPos lootPos, boolean isCorpse) {
        RigContainer rc = new RigContainer(player, RigContainer.Mode.RIG);
        RigContainer bp = new RigContainer(player, RigContainer.Mode.BACKPACK);
        int bpCols = bp.getCols();
        int bpRows = bp.getRows();
        ...
        if (isCorpse && level.getBlockEntity(lootPos) instanceof TarkovCorpseBlockEntity be) {
            ...
            for (int i = 0; i < GridInventory.MAX_CELLS; i++) {
                ...
                ItemStack s = (col < crc && row < crr && rigIdx < rc2.size()) ? rc2.get(rigIdx).copy() : ItemStack.EMPTY;
                items.add(s);
            }
            // Grid contents [151..294]
            List<ItemStack> gc = be.getGridContents();
            for (int i = 0; i < GridInventory.MAX_CELLS; i++) {
                items.add(i < gc.size() ? gc.get(i).copy() : ItemStack.EMPTY);
            }
            // Gear [295..302]: rig item, backpack item, head, chest, legs, feet, face, ear
            Map<String, ItemStack> sl = be.getSlottedItems();
            java.util.function.Function<String[], ItemStack> pick = keys -> {
                for (String k : keys) {
                    ItemStack s = sl.get(k);
            ...
            // Don't show chest armor again if it IS the rig
            if (ItemStack.matches(gCh, gRig)) gCh = ItemStack.EMPTY;
            ItemStack gLegs = pick.apply(new String[]{"armor.legs"});
            ItemStack gFeet = pick.apply(new String[]{"armor.feet"});
            ItemStack gFace = pick.apply(new String[]{"cap.face","curios.facewear"});
            ItemStack gEar  = pick.apply(new String[]{"cap.ear","curios.earwear"});
            for (ItemStack g : new ItemStack[]{gRig,gBack,gHead,gCh,gLegs,gFeet,gFace,gEar})
                items.add(g);
    ...
    public static void writeDimensions(FriendlyByteBuf buf, Player player,
        ...
        buf.writeInt(bpTmp.getCols()); buf.writeInt(bpTmp.getRows());
    ...
    private void createAllSlots() {
        ...
        //    Corpse:    [0..6] pockets row, [7..150] rig grid, [151..294] backpack grid.
        ...
        for (int i = 0; i < LOOT_SLOTS; i++) {
            ...
            if (activeLootIsCorpse) {
                if (i >= CORPSE_GEAR_BASE) {
                    // Gear items: rig, backpack, head, chest, legs, feet, face, ear
                    int gi = i - CORPSE_GEAR_BASE;
                    sx = rightPanelX + gi * CELL;
                    sy = gearY;
                } else if (i < CORPSE_POCKETS) {
                    // Pockets: one row of 7
                    ...
                    sx = (col < corpseRigCols && row < corpseRigRows)
                            ? rightPanelX + col * CELL : -2000;
                    sy = (col < corpseRigCols && row < corpseRigRows)
                            ? cRigY + row * CELL : -2000;
                } else {
                    // Backpack grid
                    ...
                    sx = (col < corpseGridCols && row < corpseGridRows)
                            ? rightPanelX + col * CELL : -2000;
                    sy = (col < corpseGridCols && row < corpseGridRows)
                            ? cGridY + row * CELL : -2000;
                }
            } else {
                // Flat container grid: first 80 slots visible
            ...
            addSlot(new Slot(lootContainer, i, sx, sy) {
                @Override public boolean isActive()                     { return fsx != -2000; }
                @Override public boolean mayPlace(@NotNull ItemStack s)  {
                    // Corpse: read-only. Container chest: allow depositing items.
                    return !isCorpseSlot && fsx != -2000;
                }
                @Override public boolean mayPickup(@NotNull Player p)    { return fsx != -2000; }
            });
        }
    }
    ...
    public void broadcastChanges() {
        ...
        if (playerInventory.player instanceof ServerPlayer sp) {
            ...
            if (cc != lastEquipChangeCount) {
                ...
                ModNetwork.CHANNEL.send(PacketDistributor.PLAYER.with(() -> sp),
                        new S2CEquipmentSyncPacket(cap.serializeNBT()));
                // Sync custom slots to their Curios slots so other mods see them
                if (com.tarkovinventory.compat.CuriosCompat.isLoaded()) {
                    ...
                    syncToCurios(sp, "back",     cap.getSlot(IPlayerEquipment.SLOT_ON_BACK));
            ...
            boolean sizeChanged = rigColsNow != lastRigCols || rigRowsNow != lastRigRows
                               || bpColsNow  != lastBpCols  || bpRowsNow  != lastBpRows;
            if (sizeChanged) scheduleMenuRefresh(sp);
    ...
    private void scheduleMenuRefresh(ServerPlayer sp) {
        ...
        sp.server.execute(() -> {
            ...
            // Preserve the cursor item through the reopen — clear it first so
            // removed() doesn't drop it, then restore it on the new menu.
            final ItemStack cursor = sp.containerMenu.getCarried().copy();
            ...
            // Restore cursor on next tick once the new menu is initialised.
            if (!cursor.isEmpty()) {
                sp.server.execute(() -> {
                    if (sp.containerMenu.getCarried().isEmpty())
                        sp.containerMenu.setCarried(cursor);
                    else
                        sp.drop(cursor, false);
                });
            }
        });
    }
    ...
    private void writeLootBack(ServerPlayer sp) {
        ...
        if (activeLootIsCorpse) {
            ...
            // Pockets [0..6]
            for (int i = 0; i < CORPSE_POCKETS; i++) be.setPocketItem(i, lootContainer.getItem(i));
            // Rig contents [7..150]
            ...
            // Gear [295..302]: write back updated items
            String[][] gearKeys = {
            ...
            for (int i = 0; i < CORPSE_GEAR_COUNT; i++) {
                ...
                for (String k : gearKeys[i]) {
                    if (sl.containsKey(k)) { sl.put(k, updated.copy()); break; }
                }
                // Always write primary key
