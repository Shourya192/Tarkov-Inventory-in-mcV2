...
public class TarkovInventoryMenu extends AbstractContainerMenu {
    ...
    public static final int POCKETS_COUNT       = 5;
    ...
    //   [8..11]   WEAPONS (melee, primary, secondary, sidearm — 4 slots)
    //   [12..16]  POCKETS (5 slots)
    //   [17..160] RIG contents grid (144 slots)
    //   [161..304] BACKPACK contents grid (144 slots)
    public  static final int CORPSE_GEAR_BASE    = 0;
    public  static final int CORPSE_GEAR_COUNT   = 12;  // 8 gear + MELEE + PRIMARY + SECONDARY + SIDEARM
    public  static final int CORPSE_POCKETS_BASE = 12;
    public  static final int CORPSE_POCKETS      = 5;
    ...
    private static final int EC1 = 10, EC3 = 162; // EC2 (center col) freed up now that KNEES/BOOTS moved off-center
    ...
    private boolean  pendingLootRefresh = false;
    ...
    private void createAllSlots() {
        ...
        int[] armorX   = {EC3, EC3, EC3, EC3};
        int[] armorY   = {0, 20, 40, 60};
        ...
        // 3. Custom equipment
        int[] custX = {EC1, EC1, EC1, -2000, EC1, -2000, EC1};
        int[] custY = {0, 20, 40, -2000, 60, -2000, 80};
        ...
        // 5. Hotbar (MELEE / PRIMARY / SECONDARY / SIDEARM / pockets)
        // Weapon slots moved below the silhouette to match the renderer's 2x2
        // block: wide PRIMARY/SECONDARY on the left, small MELEE/SIDEARM on
        // the right (see EquipmentPanelRenderer.init() for the exact layout math).
        final int weapWideW  = com.tarkovinventory.client.screen.modules.EquipmentPanelRenderer.WEAP_CELL * 2;
        final int weapSmallX = EC1 + weapWideW
                + com.tarkovinventory.client.screen.modules.EquipmentPanelRenderer.WEAP_COL_GAP;
        final int weapRow2Y  = 180
                + com.tarkovinventory.client.screen.modules.EquipmentPanelRenderer.WEAP_CELL
                + com.tarkovinventory.client.screen.modules.EquipmentPanelRenderer.WEAP_ROW_GAP;
        addSlot(new Slot(playerInventory, 1, EC1, 180));            // PRIMARY   (hotbar 1)
        addSlot(new Slot(playerInventory, 0, weapSmallX, 180));      // MELEE     (hotbar 0)
        addSlot(new Slot(playerInventory, 2, EC1, weapRow2Y));       // SECONDARY (hotbar 2)
        addSlot(new Slot(playerInventory, 3, weapSmallX, weapRow2Y));// SIDEARM   (hotbar 3)
        for (int i = 0; i < POCKETS_COUNT; i++)
            addSlot(new Slot(playerInventory, i + 4, centerPanelX + i * CELL, pocketY));
        ...
        for (int i = 0; i < LOOT_SLOTS; i++) {
            ...
            if (activeLootIsCorpse) {
                if (i < 8) {
                    ...
                    // WEAPONS row (MELEE=8, PRIMARY=9, SECONDARY=10, SIDEARM=11)
    ...
    public void broadcastChanges() {
        ...
        if (playerInventory.player instanceof ServerPlayer sp) {
            ...
            if (lootDirty && activeLootPos != null) {
                ...
                // If a rig/backpack was just placed into the corpse, its contents were
                // unpacked server-side; refresh the menu so the display slots populate.
                if (pendingLootRefresh) {
                    pendingLootRefresh = false;
    ...
    /** True if a list is null/empty or contains only empty stacks. */
    private static boolean allEmptyList(List<ItemStack> list) {
        if (list == null || list.isEmpty()) return true;
        for (ItemStack s : list) if (s != null && !s.isEmpty()) return false;
        return true;
    }
    ...
    private static boolean corpseGearSlotAccepts(int gearIdx, ItemStack s) {
        ...
        EquipmentSlotType type = switch (gearIdx) {
            ...
            case 8, 9, 10, 11 -> EquipmentSlotType.UNKNOWN; // weapons — accept any item
    ...
    private void writeLootBack(ServerPlayer sp) {
        ...
        if (activeLootIsCorpse) {
            ...
            String[][] gearKeys = {
                ...
                {"weapon.melee"}, {"weapon.primary"}, {"weapon.secondary"}, {"weapon.sidearm"}
            };
            ...
            if (rigPresent && !rigWasThere) {
                ...
                pendingLootRefresh = true; // re-read so the contents show immediately
            } else if (rigPresent) {
                // Still present. Normally re-pack the display grid into the item NBT.
                // BUT if the display grid is entirely empty while the item still has
                // stored contents, the corpse just hasn't re-displayed them yet — don't
                // wipe. Keep the existing rig contents instead.
                boolean displayEmpty = allEmptyList(rigItems);
                boolean corpseHasContents = !allEmptyList(be.getRigContents());
                ItemStack rigItem = firstNonEmpty(sl, gearKeys[0]);
                boolean itemHasNbt = rigItem.getTag() != null && rigItem.getTag().contains("TarkovRigInventory");
                if (displayEmpty && (corpseHasContents || itemHasNbt)) {
                    // Preserve: keep item NBT contents in the corpse, don't overwrite.
                    if (itemHasNbt) {
                        int pc = com.tarkovinventory.inventory.RigSizes.getCols(rigItem);
                        int pr = com.tarkovinventory.inventory.RigSizes.getRows(rigItem);
                        be.setRigContents(unpackContentsFromItem(rigItem, pc, pr), pc, pr);
                    }
                } else {
            ...
            if (bpPresent && !bpWasThere) {
                ...
                boolean displayEmpty = allEmptyList(gridItems);
                boolean corpseHasContents = !allEmptyList(be.getGridContents());
                ItemStack bpItem = firstNonEmpty(sl, gearKeys[1]);
                boolean itemHasNbt = bpItem.getTag() != null && bpItem.getTag().contains("TarkovRigInventory");
                if (displayEmpty && (corpseHasContents || itemHasNbt)) {
                    if (itemHasNbt) {
                        int pc = com.tarkovinventory.inventory.BackpackSizes.getCols(bpItem);
                        int pr = com.tarkovinventory.inventory.BackpackSizes.getRows(bpItem);
                        be.setGridContents(unpackContentsFromItem(bpItem, pc, pr), pc, pr);
                    }
                } else {
    ...
    public @NotNull ItemStack quickMoveStack(@NotNull Player player, int index) {
        ...
        if (index < GRID_MAX) {
            ...
            if (!moveItemStackTo(stack, HOTBAR_START + 4, RIG_START, false))
                return ItemStack.EMPTY;
        } else if (index >= VANILLA_ARMOR_START && index < VANILLA_ARMOR_START + VANILLA_ARMOR_COUNT) {
            // Armor → pockets
            if (!moveItemStackTo(stack, HOTBAR_START + 4, RIG_START, false)) return ItemStack.EMPTY;
        } else if (index >= CUSTOM_EQUIP_START && index < custEnd) {
            // RIG / BACKPACK / FACE / EAR / KNEES → pockets
            if (!moveItemStackTo(stack, HOTBAR_START + 4, RIG_START, false)) return ItemStack.EMPTY;
        } else if (index >= HOTBAR_START + 4 && index < RIG_START) {
