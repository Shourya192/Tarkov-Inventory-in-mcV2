        ...
        for (int i = 0; i < LOOT_SLOTS; i++) {
            ...
            addSlot(new Slot(lootContainer, i, sx, sy) {
                @Override public boolean isActive() {
                    ...
                    if (isRigContent && lootContainer.getItem(CORPSE_GEAR_BASE + 0).isEmpty()) {
                        // Move off-screen so Minecraft's renderSlot draws the dark square
                        // at (-2000,-2000) instead of the visible baked position.
                        this.x = -2000; this.y = -2000;
                        return false;
                    }
                    if (isBpContent && lootContainer.getItem(CORPSE_GEAR_BASE + 1).isEmpty()) {
                        this.x = -2000; this.y = -2000;
                        return false;
                    }
                    // Restore if gear item returned
                    this.x = fsx; this.y = fsy;
                    return true;
                }
