...
public class TarkovInventoryMenu extends AbstractContainerMenu {
    ...
    private boolean lastRigEquipped  = false;
    private boolean lastBpEquipped   = false;
    ...
    private TarkovInventoryMenu(int windowId, Inventory playerInv, ConstructorArgs a) {
        ...
        this.lastRigEquipped = rigContainer.isItemEquipped();
        this.lastBpEquipped  = backpackContainer.isItemEquipped();
    ...
    public void broadcastChanges() {
        ...
        if (playerInventory.player instanceof ServerPlayer sp) {
            ...
            // Sync capability to client whenever equipment slots change
            ...
            // Detect rig/backpack equip state change → recreate menu so slots
            // are repositioned (Slot.x/y are final and can't be moved at runtime).
            boolean rigNow = rigContainer.isItemEquipped();
            boolean bpNow  = backpackContainer.isItemEquipped();
            if (rigNow != lastRigEquipped || bpNow != lastBpEquipped) {
                lastRigEquipped = rigNow;
                lastBpEquipped  = bpNow;
