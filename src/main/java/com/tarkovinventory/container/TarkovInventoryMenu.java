...
public class TarkovInventoryMenu extends AbstractContainerMenu {
    ...
    private void writeLootBack(ServerPlayer sp) {
        ...
        if (activeLootIsCorpse) {
            ...
            // RIG ITEM handling: if the rig is still present, keep its contents packed
            // into the item NBT (so it carries them when taken). If it was just taken
            // (present before, empty now), the player already received the packed item —
            // clear the corpse's separate rig contents so they can't be taken again.
            ...
            if (rigPresent) {
                ...
                // taken (or never there) → corpse keeps no loose rig contents
