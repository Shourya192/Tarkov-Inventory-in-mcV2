...
public class TarkovInventoryMenu extends AbstractContainerMenu {
    ...
    private static ConstructorArgs parseFromBuf(Player player, @Nullable FriendlyByteBuf data) {
        if (data == null) {
            ...
            return new ConstructorArgs(rc.getCols(), rc.getRows(), bc, br,
    ...
    public void broadcastChanges() {
        ...
        if (playerInventory.player instanceof ServerPlayer sp) {
            ...
            // Refresh menu ONLY when equipping or changing to a different size.
            // When unequipping (cols→0) isActive()=false handles it — no flicker needed.
            ...
            // Only refresh when equipping new item or changing size; not on unequip
            boolean shouldRefresh = sizeChanged
                    && (rigColsNow > 0 || bpColsNow > 0)  // at least one still equipped
                    && (lastRigCols == 0 || lastBpCols == 0 // was previously unequipped
                        || rigColsNow != lastRigCols || rigRowsNow != lastRigRows   // rig size changed
                        || bpColsNow  != lastBpCols  || bpRowsNow  != lastBpRows);  // bp size changed
            lastRigCols = rigColsNow; lastRigRows = rigRowsNow;
            lastBpCols  = bpColsNow;  lastBpRows  = bpRowsNow;
            if (shouldRefresh) scheduleMenuRefresh(sp);
