...
public final class TarkovCommand {
        ...
        for (String alias : new String[]{"ti", "tarkovinventory"}) {
            dispatcher.register(
                Commands.literal(alias)
                    ...
                    .then(Commands.literal("admintags")
                        .requires(source -> source.hasPermission(2))
                        .executes(ctx -> openAdminTags(ctx.getSource())))
            );
        }
    }

    private static int openAdminTags(CommandSourceStack source) {
        try {
            ServerPlayer player = source.getPlayerOrException();
            com.tarkovinventory.network.ModNetwork.CHANNEL.send(
                    net.minecraftforge.network.PacketDistributor.PLAYER.with(() -> player),
                    new com.tarkovinventory.network.S2COpenAdminTagPacket());
            return 1;
        } catch (Exception e) {
    ...
    private static int setRigSize(CommandSourceStack source, int cols, int rows) {
        try {
            ...
            // Register the size permanently (saved to config, survives restarts)
            RigSizes.registerOverride(itemId, cols, rows);
            ...
            player.displayClientMessage(
                Component.literal("§aRegistered rig §f\"" + itemId + "\"§a → §f"
                    + cols + "§a×§f" + rows + "§a grid §7(saved permanently)."), false);
            player.displayClientMessage(
                Component.literal("§7Reopen the inventory (§f/ti§7) to see it."), false);
            player.displayClientMessage(
                Component.literal("§7To make it permanent, add to RigSizes.java:"), false);
