...
public class RigContainer implements Container {
    ...
    private ItemStack getEquippedItem() {
        if (mode == Mode.RIG) {
            // 1. Curios "body" slot (Modern Mayhem rigs)
            if (CuriosCompat.isLoaded()) {
                ItemStack cur = CuriosCompat.getSlotItem(player, "body", 0);
                if (!cur.isEmpty()) return cur;
            }
            // 2. Capability SLOT_RIG — Miklos and any item placed in our Tarkov RIG slot.
            //    Vanilla CHEST armor is intentionally NOT checked: chest armour must not
            //    accidentally open rig storage (user request).
            ...
            // BACKPACK: check our capability FIRST (master), then Curios as fallback.
            // If we clear cap.SLOT_ON_BACK but Curios "back" still has stale data,
            // we must not treat the backpack as still equipped.
            ItemStack capBack = player.getCapability(com.tarkovinventory.capability.ModCapabilities.PLAYER_EQUIPMENT)
                    ...
                    .orElse(ItemStack.EMPTY);
            if (!capBack.isEmpty()) return capBack;
            if (CuriosCompat.isLoaded()) {
    ...
    private void setEquippedItem(ItemStack stack) {
        if (mode == Mode.RIG) {
            ...
            if (CuriosCompat.isLoaded()) CuriosCompat.setSlot(player, "back", 0, stack);
        }
    }
