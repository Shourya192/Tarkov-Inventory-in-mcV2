...
public class RigContainer implements Container {
    ...
    private RigInventory loadRigInv(ItemStack item) {
        // Authoritative dimensions ALWAYS come from RigSizes/BackpackSizes for this
        // item — never from saved NBT, which can drift (e.g. from an earlier default).
        ...
        if (item.getTag() != null && item.getTag().contains("TarkovRigInventory")) {
            // Restore the stored items, but force the correct dimensions afterward.
            RigInventory inv = RigInventory.unwrapFromNBT(item.getTag().getCompound("TarkovRigInventory"));
            if (inv.getCols() != cols || inv.getRows() != rows) {
                inv.setSize(cols, rows); // re-fit items into correct grid size
            }
            return inv;
        }
    ...
    private ItemStack getEquippedItem() {
        if (mode == Mode.RIG) {
            // Check capability FIRST and ONLY — same approach as backpack.
            // Curios "body" fallback is intentionally removed: stale client-side
            // Curios data causes the rig to appear still equipped after unequip.
            return player.getCapability(com.tarkovinventory.capability.ModCapabilities.PLAYER_EQUIPMENT)
    ...
    public int getCols() {
        // Authoritative: always derive from RigSizes/BackpackSizes for the equipped
        // item so the grid width never drifts based on a stale cached inventory.
        ItemStack eq = getEquippedItem();
        if (eq.isEmpty()) return rigInventory != null ? rigInventory.getCols() : 0;
        return mode == Mode.RIG
                ? com.tarkovinventory.inventory.RigSizes.getCols(eq)
                : com.tarkovinventory.inventory.BackpackSizes.getCols(eq);
    }

    public int getRows() {
        ItemStack eq = getEquippedItem();
        if (eq.isEmpty()) return rigInventory != null ? rigInventory.getRows() : 0;
        return mode == Mode.RIG
                ? com.tarkovinventory.inventory.RigSizes.getRows(eq)
                : com.tarkovinventory.inventory.BackpackSizes.getRows(eq);
    }
}
