...
public class RigContainer implements Container {
    ...
    private ItemStack getEquippedItem() {
        if (mode == Mode.RIG) {
            // 1. Curios "body" slot (Modern Mayhem rigs, etc.) — case-insensitive via CuriosCompat
            if (CuriosCompat.isLoaded()) {
                ItemStack cur = CuriosCompat.getSlotItem(player, "body", 0);
                if (!cur.isEmpty()) return cur;
            }
            // 2. Vanilla CHEST armor slot (direct chest-armor rigs)
            ItemStack chest = player.getItemBySlot(EquipmentSlot.CHEST);
            if (!chest.isEmpty()) return chest;
            // 3. Capability SLOT_RIG — Miklos Military Armor and any item placed in
            //    our Tarkov RIG slot that has no Curios/vanilla-chest integration
            ...
            // BACKPACK
            if (CuriosCompat.isLoaded()) {
                ItemStack cur = CuriosCompat.getSlotItem(player, "back", 0);
                if (!cur.isEmpty()) return cur;
            }
            return player.getCapability(com.tarkovinventory.capability.ModCapabilities.PLAYER_EQUIPMENT)
    ...
    private void setEquippedItem(ItemStack stack) {
        if (mode == Mode.RIG) {
            if (CuriosCompat.isLoaded() && !CuriosCompat.getSlotItem(player, "body", 0).isEmpty()) {
                ...
                // Miklos / capability-only rig — write updated NBT back to SLOT_RIG
            ...
            // BACKPACK — write to capability and Curios
            player.getCapability(com.tarkovinventory.capability.ModCapabilities.PLAYER_EQUIPMENT)
        ...
        // NOTE: do NOT call broadcastChanges() here — it causes re-entrant sync issues.
        // The normal server tick loop calls broadcastChanges() every tick automatically.
    }
