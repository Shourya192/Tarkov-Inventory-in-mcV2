...
public final class EquipmentSlotTypeValidator {
     ...
     *
     * Admin-assigned types (via the tagging UI) take priority when present.
     * Otherwise falls back to the built-in heuristics (vanilla armor classes,
     * item tags, BackpackSizes/RigSizes registration, etc.) registered via
    ...
    public static boolean isValid(ItemStack stack, EquipmentSlotType type) {
        ...
        if (assigned != null) return assigned.equalsIgnoreCase(type.name());

        // No admin override — fall back to the built-in heuristics.
