...
public class RigAwareItemHandler implements IItemHandlerModifiable {
    ...
    /**
     * Wraps an existing handler (e.g. the one TaCZ passes during ammo search) and
     * appends the equipped rig's contents after it. If the given handler isn't
     * modifiable, we fall back to wrapping the player's inventory directly.
     */
    public RigAwareItemHandler(Player player, IItemHandler original) {
        this.player = player;
        if (original instanceof IItemHandlerModifiable mod) {
            this.vanilla = mod;
        } else {
            this.vanilla = new InvWrapper(player.getInventory());
        }
