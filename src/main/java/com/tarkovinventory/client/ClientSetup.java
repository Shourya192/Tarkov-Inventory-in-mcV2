...
public class ClientSetup {
    ...
    public static void onAddLayers(EntityRenderersEvent.AddLayers event) {
        for (String skin : event.getSkins()) {
            try {
                var raw = event.getSkin(skin);
                if (raw == null) continue;
                // Cast carefully — on some platforms the renderer uses a different
                // PlayerModel subtype. Only proceed if the model IS a PlayerModel.
                if (!(raw.getModel() instanceof net.minecraft.client.model.PlayerModel)) continue;
                ...
                // Skin renderer incompatible — skip silently
            }
        }
    }
