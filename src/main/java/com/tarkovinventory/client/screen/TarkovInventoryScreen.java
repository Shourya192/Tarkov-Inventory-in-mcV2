...
public class TarkovInventoryScreen extends AbstractContainerScreen<TarkovInventoryMenu> {
    ...
    /**
     * Expands the clickable/hover area for the weapon slots to match their
     * larger visual box (WEAP_CELL, and double-width for the PRIMARY/SECONDARY
     * wide boxes) instead of vanilla's fixed 16x16 — otherwise only the small
     * top-left corner (where the old 16x16 hitbox was) would be grabbable.
     */
    @Override
    protected boolean isHovering(Slot slot, double mouseX, double mouseY) {
        if (isWeaponHotbarSlot(slot)) {
            ...
            return relX >= 0 && relX < w && relY >= 0 && relY < h;
        }
