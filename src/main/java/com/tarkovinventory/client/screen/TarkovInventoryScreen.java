...
public class TarkovInventoryScreen extends AbstractContainerScreen<TarkovInventoryMenu> {
    ...
    /**
     * Loot slots use our styled cell background (drawn by RightInventoryPanelRenderer).
     * Skip vanilla slot border; only draw the item itself.
     */
    @Override
    protected void renderSlot(@NotNull GuiGraphics g, @NotNull Slot slot) {
        if (slot.container == menu.getLootContainer()) {
            if (!slot.isActive() || slot.getItem().isEmpty()) return;
            int sx = leftPos + slot.x;
            int sy = topPos + slot.y;
            g.renderItem(slot.getItem(), sx, sy);
            g.renderItemDecorations(font, slot.getItem(), sx, sy);
        } else {
