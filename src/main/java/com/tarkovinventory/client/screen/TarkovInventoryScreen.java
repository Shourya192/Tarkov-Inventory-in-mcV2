...
public class TarkovInventoryScreen extends AbstractContainerScreen<TarkovInventoryMenu> {
    ...
    public void render(@NotNull GuiGraphics g, int mouseX, int mouseY, float partialTick) {
        ...
        // Paint over phantom dark squares Minecraft draws for inactive corpse content slots
        coverInactiveLootSlots(g);
        drawSearchOverlay(g);
        renderTooltip(g, mouseX, mouseY);
    }

    /** Covers phantom dark squares Minecraft renders for inactive slots at visible positions. */
    private void coverInactiveLootSlots(GuiGraphics g) {
        ...
        for (int i = TarkovInventoryMenu.CORPSE_RIG_BASE; i < TarkovInventoryMenu.LOOT_SLOTS; i++) {
            ...
            if (slot.isActive() || slot.x < 0) continue;
