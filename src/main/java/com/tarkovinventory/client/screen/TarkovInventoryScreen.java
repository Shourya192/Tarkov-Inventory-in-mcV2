...
public class TarkovInventoryScreen extends AbstractContainerScreen<TarkovInventoryMenu> {
    ...
    protected void renderBg(@NotNull GuiGraphics g, float partialTick, int mouseX, int mouseY) {
        ...
        equipment.render(g, true, mouseX, mouseY);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        // Weapon slots have a bigger visual box than their real 16x16 hitbox
        // (see renderWeaponSlotItemsScaled). isHovering() is private in this
        // MC version so it can't be overridden — instead we detect clicks in
        // the expanded box ourselves and route them straight to slotClicked.
        for (Slot slot : menu.slots) {
            if (!isWeaponHotbarSlot(slot)) continue;
            int idx = slot.getContainerSlot();
            boolean wide = idx == 1 || idx == 2; // PRIMARY / SECONDARY
            int w = wide ? EquipmentPanelRenderer.WEAP_CELL * 2 : EquipmentPanelRenderer.WEAP_CELL;
            int h = EquipmentPanelRenderer.WEAP_CELL;
            double relX = mouseX - (leftPos + slot.x);
            double relY = mouseY - (topPos + slot.y);
            if (relX >= 0 && relX < w && relY >= 0 && relY < h) {
                ...
                slotClicked(slot, slot.index, button, type);
                return true;
            }
        }
    ...
    /**
     * Scales up item rendering for the weapon slots (MELEE/PRIMARY/SECONDARY/
     * SIDEARM) so the icon fills the larger 32px boxes instead of sitting as
     * a small 16px icon in the corner. Vanilla always renders item icons at a
     * fixed 16x16 via a private method we can't override, so instead we draw
     * an enlarged copy on top, anchored at the same top-left corner, which
     * fully covers the small icon underneath.
     */
    private void renderWeaponSlotItemsScaled(GuiGraphics g) {
        float scale = EquipmentPanelRenderer.WEAP_CELL / 16f;
        for (Slot slot : menu.slots) {
            if (isWeaponHotbarSlot(slot) && slot.hasItem()) {
                ItemStack stack = slot.getItem();
                // slot.x/y are menu-relative — must add leftPos/topPos to get
                // actual screen coordinates (this was the bug: without the
                // offset we were drawing near the screen origin instead of
                // over the real slot, so the small vanilla icon stayed visible).
    ...
    /** True for the 4 weapon slots — backed by player-inventory hotbar indices 0-3. */
    private boolean isWeaponHotbarSlot(Slot slot) {
        return slot.container instanceof Inventory && slot.getContainerSlot() < 4;
    }

    /**
     * Expands the clickable area for the weapon slots — see the mouseClicked
     * override above, which does the actual hit-testing (isHovering() itself
     * is private in this MC version and can't be overridden).
     */
    ...
    public void render(@NotNull GuiGraphics g, int mouseX, int mouseY, float partialTick) {
        renderBackground(g);
        // Update live dimensions AND equipped state every frame. Pass 0 when not
        // equipped so the panel background shrinks to fit only visible sections.
        boolean rigEq = menu.getRigContainer().isItemEquipped();
        boolean bpEq  = menu.getBackpackContainer().isItemEquipped();
        grid.setRigDimensions(
                rigEq ? menu.getRigContainer().getCols() : 0,
                rigEq ? menu.getRigContainer().getRows() : 0);
        grid.setBackpackDimensions(
                bpEq ? menu.getBackpackContainer().getCols() : 0,
                bpEq ? menu.getBackpackContainer().getRows() : 0);
        grid.setEquippedState(rigEq, bpEq);
        grid.updateHover(mouseX, mouseY);
        equipment.updateHover(mouseX, mouseY);
        super.render(g, mouseX, mouseY, partialTick);
        renderWeaponSlotItemsScaled(g);
    ...
    private void coverAllInactiveSlots(GuiGraphics g) {
        // Only cover CORPSE loot content slots (rig/backpack inside corpse) that
        // go inactive when their gear item is taken. The main rig/backpack grid
        // slots are handled by the InventoryGridRenderer (it skips drawing cells
        // when not equipped), so we must NOT paint over those — doing so created
        // the big black box.
        int lootBase = menu.getLootSlotBase();
        if (lootBase < 0 || !menu.activeLootIsCorpse) return;
        for (int i = TarkovInventoryMenu.CORPSE_RIG_BASE; i < TarkovInventoryMenu.LOOT_SLOTS; i++) {
            var slot = menu.slots.get(lootBase + i);
            if (slot.isActive() || slot.x < 0 || slot.y < 0) continue;
            g.fill(leftPos + slot.x - 1, topPos + slot.y - 1,
                   leftPos + slot.x + 17, topPos + slot.y + 17, 0xFF1A1A1A);
        }
    }
