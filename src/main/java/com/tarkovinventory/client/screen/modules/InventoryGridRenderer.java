...
public class InventoryGridRenderer {
    ...
    /** Backpack panel grid start Y. Uses a fixed reserved rig area so it always
     *  matches the baked slot positions in TarkovInventoryMenu. */
    public int getBackpackY() {
        int reservedRigRows = 3; // RigSizes.DEFAULT_ROWS — must match menu's gridBaseY
    ...
    public void renderWithBackground(GuiGraphics g) {
        // Backpack slots are baked in the menu at a FIXED position that reserves
        // 3 rig rows of space (RigSizes.DEFAULT_ROWS), so the panel layout must
        // reserve that same space to keep slots and rendered cells aligned.
        int reservedRigRows = 3;
        int width = Math.max(Math.max(bpEquipped ? bpCols : 0, rigEquipped ? rigCols : 0), 7) * CELL;

        int y = top;
        y += CELL;                                            // pockets row
        y += SECTION_GAP + LABEL_H + reservedRigRows * CELL;  // rig label + reserved rig area
        // backpack section only adds height when a backpack is equipped
        if (bpEquipped && bpRows > 0) {
            y += SECTION_GAP + LABEL_H + bpRows * CELL;       // backpack label + grid
        } else {
            y += SECTION_GAP + LABEL_H;                       // just the backpack label
        }
