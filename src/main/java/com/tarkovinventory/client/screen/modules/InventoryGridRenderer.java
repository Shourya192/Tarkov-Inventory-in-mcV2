 ...
 *   1. Pockets row  — 5 pocket slots (Forge hotbar slots 4-8)
...
public class InventoryGridRenderer {
    ...
    public String getRegionAt(double mouseX, double mouseY) {
        int py = getPocketsY();
        if (mouseY >= py && mouseY < py + CELL && mouseX >= left && mouseX < left + 5 * CELL)
            return "pockets";
    ...
    public void renderWithBackground(GuiGraphics g) {
        ...
        int width = Math.max(Math.max(effBpCols, effRigCols), 5) * CELL;
