...
public class InventoryGridRenderer {
    ...
    public void render(GuiGraphics g) {
        ...
        if (rigCols > 0 && rigRows > 0) {
            for (int row = 0; row < rigRows; row++) {
                for (int col = 0; col < rigCols; col++) {
                    int idx = row * rigCols + col;
                    if (rigEquipped) drawSlotCell(g, left + col * CELL, rigY + row * CELL, hoverRigSlot == idx);
                    else             drawEmptySlot(g, left + col * CELL, rigY + row * CELL);
                }
            }
        }
        ...
        if (bpCols > 0 && bpRows > 0) {
            for (int row = 0; row < bpRows; row++) {
                for (int col = 0; col < bpCols; col++) {
                    int idx = row * 12 + col;
                    if (bpEquipped) drawSlotCell(g, left + col * CELL, bpY + row * CELL, hoverGridSlot == idx);
                    else            drawEmptySlot(g, left + col * CELL, bpY + row * CELL);
                }
            }
        }
    }
