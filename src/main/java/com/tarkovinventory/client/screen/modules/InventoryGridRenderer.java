...
public class InventoryGridRenderer {
    ...
    public void renderWithBackground(GuiGraphics g) {
        ...
        // Stack sections explicitly. Each section only takes space if its item is equipped.
        // pockets (always) → rig label+grid (if rig) → backpack label+grid (if backpack)
        int y = top;
        y += CELL;                                    // pockets row
        if (rigEquipped) {
            y += SECTION_GAP + LABEL_H;               // rig label
            y += rigRows * CELL;                      // rig grid
        }
        if (bpEquipped) {
            y += SECTION_GAP + LABEL_H;               // backpack label
            y += bpRows * CELL;                       // backpack grid
        }
