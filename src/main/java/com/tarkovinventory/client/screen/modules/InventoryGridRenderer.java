...
public class InventoryGridRenderer {
    ...
    public void renderWithBackground(GuiGraphics g) {
        ...
        int width = Math.max(Math.max(rigEquipped ? rigCols : 0, bpEquipped ? bpCols : 0), 7) * CELL;

        // Height extends only as far as the equipped sections require.
        int pocketsBottom = getPocketsY() + CELL;
        int rigBottom     = rigEquipped ? getRigGridY() + rigRows * CELL : pocketsBottom;
        int bpBottom      = bpEquipped  ? getBackpackY() + bpRows * CELL : rigBottom;
        int bottom        = Math.max(Math.max(pocketsBottom, rigBottom), bpBottom);
        int totalHeight   = bottom - top;

        // Outer panel background — sized to fit only the visible content
