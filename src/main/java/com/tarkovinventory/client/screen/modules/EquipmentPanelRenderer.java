...
public class EquipmentPanelRenderer {
    ...
    public void init(int left, int top) {
        ...
        int weaponX = x + 10;
        addVisual("PRIMARY",   EquipmentSlotType.WEAPON, weaponX, weaponY, weaponW, CELL);
        addVisual("SECONDARY", EquipmentSlotType.WEAPON, weaponX, weaponY + CELL + GAP, weaponW, CELL);
    }
    ...
    public void render(GuiGraphics g, boolean renderSilhouette) {
        ...
        // Separator line + WEAPONS label above the wide weapon slots (row 7 = y+140)
        int weaponSepY = y + 129;
    ...
    private void renderSlotLabel(GuiGraphics g, EquipmentSlot v) {
        ...
        if (v.type == EquipmentSlotType.WEAPON) {
            // Wide slot — text inside on the left, greyed out (item overdraws it)
            g.drawString(mc.font, label, v.x1 + 4, centY, 0xFF505050, false);
