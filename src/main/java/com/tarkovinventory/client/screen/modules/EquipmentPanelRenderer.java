...
public class EquipmentPanelRenderer {
     ...
     *  Facing rotation only — no mouse-tracking, matches the static look
     *  the silhouette used to have. Tune MODEL_SCALE / MODEL_Y_OFFSET
     *  in-game if the model doesn't sit centered in the panel. */
    private static final int MODEL_SCALE    = 42;
    private static final int MODEL_Y_OFFSET = 150; // from panel top to the model anchor point
    private static final Quaternionf MODEL_ROTATION = new Quaternionf().rotateZ((float) Math.PI);
    private static final Quaternionf MODEL_ROTATION_X = new Quaternionf(); // identity — no head tilt
    ...
    public void init(int left, int top) {
        ...
        int weaponY = y + 180;
        int weaponX = x + 10;
        int wideW   = CELL * 2 + GAP; // wide box spans two normal cells
        int smallX  = weaponX + wideW + GAP;
        int row2Y   = weaponY + CELL + GAP;
        addVisual("PRIMARY",   EquipmentSlotType.WEAPON, weaponX, weaponY, wideW,  CELL);
        addVisual("MELEE",     EquipmentSlotType.WEAPON, smallX,  weaponY, CELL,   CELL);
        addVisual("SECONDARY", EquipmentSlotType.WEAPON, weaponX, row2Y,   wideW,  CELL);
        addVisual("SIDEARM",   EquipmentSlotType.WEAPON, smallX,  row2Y,   CELL,   CELL);
    }
        ...
        if (renderSilhouette) {
            ...
            if (entity != null) {
                ...
                InventoryScreen.renderEntityInInventory(
                        ...
                        MODEL_ROTATION, MODEL_ROTATION_X, entity);
            }
        }
