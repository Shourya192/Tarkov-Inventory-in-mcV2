...
public class EquipmentPanelRenderer {
    ...
    private static final int MODEL_Y_OFFSET = 110; // from panel top to the model anchor point (moved up)
    ...
    public void init(int left, int top) {
        ...
        // Row 3: KNEES (col1) + BOOTS (col3) — moved off the center column so
        // they no longer sit underneath the player model.
        addVisual("KNEES",  EquipmentSlotType.KNEE, col1X, y + row * vStep, CELL, CELL);
        addVisual("BOOTS",  EquipmentSlotType.BOOTS, col3X, y + row * vStep, CELL, CELL);
        row++;
    ...
    public void render(GuiGraphics g, boolean renderSilhouette, int mouseX, int mouseY) {
        ...
        if (renderSilhouette) {
            ...
            if (entity != null) {
                ...
                // Match vanilla's own inventory-screen player preview: only a
                // slight up/down head tilt based on cursor height, no left/right
                // yaw (a yaw component here made the model appear to face away).
                Quaternionf bodyRot = new Quaternionf().rotateZ((float) Math.PI);
                Quaternionf headRot = new Quaternionf().rotateX((float) (dy / 40.0));
                bodyRot.mul(headRot);

                InventoryScreen.renderEntityInInventory(
                        g, modelX, modelY, MODEL_SCALE,
                        bodyRot, headRot, entity);
