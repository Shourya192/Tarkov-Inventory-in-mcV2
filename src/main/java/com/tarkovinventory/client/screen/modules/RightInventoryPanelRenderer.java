...
public class RightInventoryPanelRenderer {
    ...
    public void render(GuiGraphics g, int left, int top,
        ...
        if (isCorpse) {
            ...
            // ── RIG grid — only shown while rig item is in corpse ────
            ...
            // ── BACKPACK grid — only shown while backpack item is in corpse ─
            int bpY = rigY + (corpseHasRig ? corpseRigRows * CELL + SECTION_GAP + LABEL_H : 0);
            if (corpseHasBp) {
